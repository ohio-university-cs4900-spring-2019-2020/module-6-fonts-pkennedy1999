...
void GLViewNewModule::onCreate()
{
   ...
   WOFTGLString* worldString = WOFTGLString::New(ManagerEnvironmentConfiguration::getSMM() + "/fonts/ARIBLK.TTF", 36);
   worldString -> setPosition(100, 0, 10);
   worldString -> setText("Look Left");
   worldString -> getModel() -> setLookDirection(Vector(0, -1, 0));
   worldString -> getModel() -> setNormalDirection(Vector(-1, 0, 0));
   worldString -> getModel() -> setScale(Vector(100, 100, 100));
   worldLst -> push_back(worldString);

   this -> text = WOFTGLString::New(ManagerEnvironmentConfiguration::getSMM() + "/fonts/ARIBLK.TTF", 36);
   text -> setPosition(0, 100, 25);
   text -> setText("Press 2 to type: ");
   text -> getModel() -> setLookDirection(Vector(1, 0, 0));
   text -> getModel() -> setNormalDirection(Vector(0, -1, 0));
   text -> getModel() -> setScale(Vector(100, 100, 100));
   worldLst -> push_back(text);

   this-> clone = CloneTrooper::New();
   worldLst->push_back(this->clone->getClone());

   //engine = new PhysicsCreate();
}
...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   if(this -> isTyping) {
	...
	char c = key.keysym.sym;
	    ...
	    this -> client->sendNetMsgSynchronousTCP(NetMsgText(this->text->getText()));
        } 
